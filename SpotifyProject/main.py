...
def display_artist_info():
    ...
    print("\nTop Tracks:")
    for idx, song in enumerate(get_songs_by_artist(token, artist["id"]), 1):
        print(f"{idx}. {song['name']}")

    print("\nAlbums:")
    for album in get_albums_by_artist(token, artist["id"]):
        print(album['name'])

    print("\nRelated Artists:")
    for related_artist in get_related_artists(token, artist["id"]):
